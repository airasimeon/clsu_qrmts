...
<body>
	...
	<div class="details">
		...
		<div class="container col-md-8">
			<form  method="POST">
			<?php
			$costumer = mysqli_query($conn,"SELECT * FROM costumer WHERE token='$kode'"); 
			...
			if(isset($_POST['submit'])){
				...
				// for ($i=0; $i <= count($costumer) ; $i++) { 

				// 	$conn->query("UPDATE costumer SET kursi='$kursi[$i]' WHERE token='$kode'");
				// }
				foreach ($costumer as $cus) {
					# code...
					
						// $conn->query("UPDATE costumer SET kursi='$kur' WHERE token='$kode'");
					// $cus['kursi'] = $kursi[$count];
					// $conn->query("UPDATE $cus");
					// $count++;
					 $conn->query("UPDATE costumer SET kursi='$kursi[$count]' WHERE token='$kode' and id = '$cus[id]' ");
					 $count++;
				}header('Location:data.php?'.$get_data);
			...
			?>
				...
				<div class="form-group" style="float:left;margin:10px;">
					<label label class="checkbox-inline">
						<input type="checkbox" value="<?php echo $i ?>" name="kursi[]" id="kursi"><?php echo $i ?>
					</label>
				</div>
			<?php
				} 
			?>
			<div class="clearfix"></div>
			<div class="a" style="padding-top:30px;float:right;">
				<input type="submit" class="btn btn-primary" name="submit" value="Submit" style="width:100px;">
			</div>
			</form>
		</div>
		<div class="clearfix"></div>
...
<script type="text/javascript">
	var limit = <?php echo $jumlah_cos ?>-1;


	var kode = '<?php echo $_GET['kode'] ?>';


	$('input[id="kursi"]').on('change',function(){

		if( $('input[id="kursi"]:checked').length >= limit ){

				if( kode != null ){
			 		$('input[id="kursi"]').not(':checked').attr('disabled',true);

			 		if(  $('input[id="kursi"]:checked').length <= limit ){
			 			$('input[id="kursi"]').removeAttr('disabled',true);
